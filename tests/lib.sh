...
run_install() {
  HOME="${FAKE_HOME}" PATH="${FAKE_BIN}:${PATH}" \
    ...
    DOTFILES_SKIP_PACKAGE_PROMPT="${DOTFILES_SKIP_PACKAGE_PROMPT:-0}" \
    DOTFILES_SKIP_CONFIG_PROMPT="${DOTFILES_SKIP_CONFIG_PROMPT:-0}" \
    DOTFILES_INSTALL_PACKAGES="${DOTFILES_INSTALL_PACKAGES:-0}" \
    bash "${INSTALL_SCRIPT}"
}

run_install_config() {
  DOTFILES_SKIP_PACKAGE_PROMPT=1 DOTFILES_SKIP_CONFIG_PROMPT=1 run_install
}
